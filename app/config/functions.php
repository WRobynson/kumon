...
{
	return "&emsp;<1 class='<i class='fa-solid fa-question dica' data-bs-toggle='tooltip' data-bs-html='true' data-bs-placement='right' title='{$txt}'></i>";
}
...
/**
 * Calcula a diferença em dias entre uma data fornecida e a data atual.
 * 
 * @param string $data A data no formato 'Y-m-d' a ser comparada com a data atual.
 * @return int O número de dias de diferença. Retorna positivo se a data for no passado e negativo se no futuro.
 */
function haQuantosDias($data) {
	...
	$diferenca = $hoje->diff($dataInformada)->days; // Diferença em dias (valor absoluto)
	$diferencaDias = $hoje > $dataInformada ? $diferenca : -$diferenca;
